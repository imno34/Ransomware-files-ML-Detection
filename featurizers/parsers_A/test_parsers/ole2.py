# parsers/ole2_dir_parser_fixed.py
# TSV-вывод:
# filename  ole_dir_ok  ole_stream_count  ole_fat_ok  ole_mini_fat_ok  ole_root_entry_present  ole_summaryinfo_present  ole_expected_streams_present
...
def parse_header(data: bytes):
    ...
    # ВАЖНО: корректные смещения согласно спецификации CFB
    ...
    # 0x34 — transaction signature (не используем)
...
def build_fat(data: bytes, H: dict):
    ...
    while difat_sect not in (FREESECT, ENDOFCHAIN) and difat_remaining > 0 and len(visited) < MAX_SECTORS_READ:
        ...
        for s in entries:
            if s != FREESECT:
                fat_sector_indices.append(s)
        difat_sect = u32(buf, sector_size - 4)  # next DIFAT sector
        difat_remaining -= 1
...
# --- Признаки MiniFAT (лайтовая проверка) ---
...
def parse_ole2_features_one(path: str):
...
def process_directory(dir_path: str):
    ...
    for file_path in Path(dir_path).glob("*.*"):
        if not file_path.is_file():
            continue
        try:
            feats = parse_ole2_features_one(str(file_path))
            if feats is None:
                continue
            row = {"file": file_path.name, **feats}
            print("\t".join(str(row[c]) for c in cols))
        except Exception:
            # минимализм: падающие файлы просто пропускаем
            pass

if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python ole2_dir_parser_fixed.py <directory_path>")
        sys.exit(1)
