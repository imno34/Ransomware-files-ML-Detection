...
# Разрешенные исходные классы для формирования контрольного augmentation pool.
...
def ensure_augmented_dirs(input_csv: Path) -> tuple[Path, Path]:
    # Создание директорий для выборки под контрольный тест.
...
def load_excluded_pair_ids(exclusions_csv: Path) -> Set[str]:
    # Загрузка списка исключений из колонки aug_parent.
    if not exclusions_csv.is_file():
        raise FileNotFoundError(f"Exclusions CSV not found: {exclusions_csv}")

    df = pd.read_csv(exclusions_csv)
    if "aug_parent" not in df.columns:
        raise ValueError(f"Exclusions CSV must contain column 'aug_parent': {exclusions_csv}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build independent augmentation split with exclusion list.")
    parser.add_argument(
        "--incsv",
        type=Path,
        required=True,
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Path to CSV with exclusion list in column 'aug_parent'.",
    )
    parser.add_argument(
        "--froot",
        type=Path,
    ...
    exclusions_csv = args.excsv.resolve()
    files_root = args.froot.resolve()
    if not files_root.is_dir():
        raise FileNotFoundError(f"Files root directory not found: {files_root}")

    excluded_pair_ids = load_excluded_pair_ids(exclusions_csv)

    rng = random.Random(DEFAULT_SEED)
    df = pd.read_csv(input_csv)
    for required in ("path", "gt_class", "orig_ext", "pair_id"):
    ...
    # 3) Исключение по соответствию pair_id -> aug_parent.
    before_exclusion = len(pool_df)
    if excluded_pair_ids:
        pair_ids = pool_df["pair_id"].fillna("").astype(str).str.strip()
        pool_df = pool_df[~pair_ids.isin(excluded_pair_ids)].copy()
    excluded_rows = before_exclusion - len(pool_df)

    # 4) Квотный отбор до заданной доли augmentation pool.
    ...
    # 5) Создание каталога augmented/files_to_augment.
    augmented_root, files_to_augment_dir = ensure_augmented_dirs(input_csv)

    # 6) Копирование выбранных файлов в files_to_augment без изменений.
    copied = 0
    ...
    print(f"Eligible pool rows: {before_exclusion}")
    print(f"Excluded by pair_id/aug_parent: {excluded_rows}")
    print(f"Pool rows after exclusions: {len(pool_df)}")
